...
public class PayDemoActivity extends FragmentActivity {
    ...
    private TextView prepayment, biaoshu, product_subject;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        prepayment = (TextView) findViewById(R.id.product_price);
        ...
        try {
            //预支付
            one = Double.parseDouble(MemoryBean.prepayment);
        } catch (Exception e) {
            one = 0.0;
        }

        String couponMoney = "0";
        if (getIntent().getStringExtra("couponMoney") != null) {
            //优惠券
            couponMoney = getIntent().getStringExtra("couponMoney");
        }
        double two;
        try {
            two = Double.parseDouble(couponMoney);
        } catch (Exception e) {
            two = 0.0;
        }

        prepaymentZhiFuBao = df.format(one) + "";
        prepayment.setText(prepaymentZhiFuBao + "元");

        TextView yufukuai = (TextView) findViewById(R.id.yufukuai);
        yufukuai.setText(one + "元");
        TextView youhuiquan = (TextView) findViewById(R.id.youhuiquan);
        youhuiquan.setText(two + "元");

        biaoshu = (TextView) findViewById(R.id.biaoshu);
        biaoshu.setText(getIntent().getStringExtra("orderNo"));

        product_subject = (TextView) findViewById(R.id.product_subject);
        product_subject.setText(getIntent().getStringExtra("paotuiName"));
    ...
    private void ailpay() {
        ...
        String orderInfo = getOrderInfo("优享生活" + MemoryBean.projectName,
                "优享生活服务订单",
                prepaymentZhiFuBao);
...
//		orderInfo += "&out_trade_no=" + "\"" + getOutTradeNo() + "\"";
        orderInfo += "&out_trade_no=" + "\"" + biaoshu.getText().toString() + "\"";
...
//		orderInfo += "&notify_url=" + "\"" + "http://notify.msp.hk/notify.htm" + "\"";
        ...
        orderInfo += "&notify_url=" + "\"" + getIntent().getStringExtra("notifyUrl") + "\"";
//		System.out.println(getIntent().getStringExtra("notifyUrl"));
...
//		orderInfo += "&return_url=\""+getIntent().getStringExtra("notifyUrl")+"\"";
